module VAST
  ...
  class Ad < Element
    ...
    def error_url
      ...
      Addressable::URI.parse(error_url_node.content.strip) if error_url_node
    end
    ...
    def impression
      Addressable::URI.parse(source_node.at('Impression').content.strip)
    end
    ...
    def impressions
      source_node.xpath('.//Impression').to_a.collect do |node|
        Addressable::URI.parse(node.content.strip)
      end
    end
